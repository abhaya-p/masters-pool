...
def fetch_data():
    ...
    max_round_1_score = df.loc[(df['round_Id']==1) & (df['status']=='active') & (df['isAmateur']!=True), 'round_score'].max()
    ...
    max_round_2_score = df.loc[(df['round_Id']==2) & (df['status']=='active') & (df['isAmateur']!=True), 'round_score'].max()
